...
with open(r'C:\Users\wsy\Desktop\AFAC\测试集.json', 'rb') as f:
    train_text = json.load(f)

hangye = train_text['行业']
weidu = train_text['基金信息维度']
zhi = train_text['基金信息值']
huashu = train_text['利好事件']
...
with open('test.json', 'w+', encoding='utf-8') as f:
    for id in list(map(lambda x: str(x), hangye_ids)):
        ...
        text_jijin = '#'.join(['行业*'+hang_text, "维度*"+wei_text, "值*"+zhi_text])
        text_lihao = '#'.join(['行业*'+hang_text, "维度*"+'利好事件', "值*"+hua_text])
        for _ in range(5):
            rowdata_jijin = {'text': text_jijin, 'answer':text_jijin}
            json_rowdata = json.dumps(rowdata_jijin, indent=0, ensure_ascii=False).replace('\n','') + '\n'
            f.write(json_rowdata)
        for _ in range(5):
            rowdata_lihao = {'text': text_lihao, 'answer':text_lihao}
            json_rowdata = json.dumps(rowdata_lihao, indent=0, ensure_ascii=False).replace('\n','') + '\n'
            f.write(json_rowdata)
print('f')
