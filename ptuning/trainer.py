...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                train_loss.append(tr_loss_step)
                ...
                if total_batched_samples % args.gradient_accumulation_steps == 0 or (
                    ...
                    # 模型更新后，每隔args.eval_accumulation_steps做一次验证集的evaluate
                    if self.state.global_step % args.eval_accumulation_steps == 0:
                        eval_loss, _ = self.evaluate(self.eval_dataset)
                        eval_losses.append(eval_loss)
                        train_losses.append(sum(train_loss) / len(train_loss))
                        logger.info(f"\nepoch{self.state.global_step} train_loss:{sum(train_loss) / len(train_loss)}  eval_loss:{eval_loss}")
                        print(f"\nepoch{self.state.global_step} train_loss:{sum(train_loss) / len(train_loss)}  eval_loss:{eval_loss}")
                    model.zero_grad()
        ...
        return train_losses, eval_losses, TrainOutput(self.state.global_step, train_loss, metrics)
    ...
    def evaluation_loop(
        ...
        return sum(eval_loss) / len(eval_loss), EvalLoopOutput(predictions=all_preds, label_ids=all_labels, metrics=metrics, num_samples=num_samples)
